...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        if "servo2" in data:
            angle_servo1 = int(data["servo2"])
            
            # Validasi sudut (0 - 180 derajat)
            if 0 <= angle_servo1 <= 180:
                duty_cycle = angle_to_duty_cycle(angle_servo1)
                print(f"Servo2 bergerak ke {angle_servo1} derajat dengan duty cycle {duty_cycle:.2f}%")
        ...
        print("Data untuk servo2 bukan angka valid.")
...
def main():
    ...
    PWM_pin = 3               # Pin yang digunakan (sesuaikan dengan pin yang Anda gunakan)
